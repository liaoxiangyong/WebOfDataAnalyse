...
class ListConfigs extends React.Component {
    ...
    handleClick(e) {
        console.log('click ', e);
        this.props.getConfigChange(e.key,this.props.configs,this.props.modelDetail);
    }
    ...
    render() {
        ...
        let configArrays = _.groupBy(this.props.configs, 'fileId');
        let keyArray = [];
        for(var key in configArrays){
            keyArray.push(key);
        }
        ...
        let configs = _.map(nowConfig,(fileId)=>{
            ...
            let nowFile=this.props.configs.filter(item=>(item.fileId===fileId))[0];
            return(
        ...
        return (
            <div className="menu-page">
                ...
                <Pagination simple current={this.props.currentConfigPage} pageSize={myPageSize} onChange={this.onChange}  total={keyArray.length} className="pagination"/>
            </div>
        );
    }
}
